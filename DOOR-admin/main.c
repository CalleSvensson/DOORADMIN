...
void FakeTest(SYSTEM_STATE* state) {
	while (true) {
		printf("\nPlease scan card to enter or X to go back\n");
		printf("CURRENTLY LAMP IS: Off. ");
		int checkCard;
		char scan[5];
		GetInput("\nCard Number", &scan, 5);
		if (strlen(scan) == 1 && scan[0] == 'x' || strlen(scan) == 1 && scan[0] == 'X'){
			system("cls");
			return;
		}
		else {
			checkCard = charToInt(scan);
			if (checkCard==NULL){
				system("cls");
				printf("\nNot a number");
			}
			else{
			CARD* cardToCheck = GetCardNum(checkCard, state);
			if (cardToCheck == NULL) {
				system("cls");
				printf("\nCard not in system.");
			}
				...
				system("cls");
				printf("\nCURRENTLY LAMP IS: %s. ", cardToCheck->access ? "Green" : "Red");
				Sleep(3000);
				system("cls");
			}
			}
